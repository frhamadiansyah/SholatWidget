...
class SholatViewModelTests: XCTestCase {
    ...
    func testSholatViewModel_WhenFetchSholatTimingsSuccessful_ShouldReturnsTimings() {
        ...
        sut.mapTodaysTiming(withModel: requestModel, todaysDate: Date(timeIntervalSince1970: 1599000000))
        self.wait(for: [myExpectation], timeout: 10)

        XCTAssertNotNil(sut.sholatSchedule, "fetchTodaysTiming() method did not return a timing object. Should have return an timing object")
//        XCTAssertEqual(sut.sholatSchedule?.schedule[.fajr], "04:09 (BST)", "fetchTodaysTiming() method fetch the wrong day")
    ...
    func testSholatViewModel_WhenGivenDifferentDateFromResponse_ShouldReturnsError() {
        let myExpectation = expectation(description: "Expected the fetchSholat() method to be called")
        mockSholatViewDelegate.expectation = myExpectation
        sut.mapTodaysTiming(withModel: requestModel, todaysDate: Date(timeIntervalSince1970: 1499000000))
        self.wait(for: [myExpectation], timeout: 10)
